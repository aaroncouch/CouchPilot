---
description: Apply the project's Python style conventions when writing or reviewing code.
---
...
For human-facing prose in Python source, `writing-voice` and
...
It does not govern agent-facing artifacts, session files, prompts, or reports.

Every pair below is a real pylint check unless marked otherwise. When a check
triggers, refactor the code until it passes. Inline disables (`# pylint:
disable=`, `# noqa`, `# type: ignore`) require explicit operator authorization
— see the linter disable gate in the `python` rule.
...
and this fires, stop and request operator authorization. The fix belongs in
project config, not in the source file.
