---
description: Review test changes for integrity pitfalls and meaningful coverage.
---
...
| Diagnosis | Required Action | Prohibited Shortcut |
|---|---|---|
| `implementation-defect` | Fix the code to satisfy the general invariant | Hardcoding fixture values, test-only branches, or one-input special cases |
| `incorrect-test` | Fix the test; cite an authoritative source other than the current implementation | Loosening assertions, changing expected values to observed ones, `skip`/`xfail`, or mocking the unit under test |
| `wrong-assumption` | Stop and report; the task needs restating | Implementing anyway |
| `architecture-conflict` | Stop and escalate; operator decision needed | Forcing the suite green despite contract conflict |

Reaching for `implementation-defect` because it lets you keep working is how a special case gets written.
